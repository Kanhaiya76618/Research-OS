...
const RELATION_LABELS: Record<GraphEdge['relation'], string> = {
  'builds-on': 'Supply Dependency',
  'cites': 'Subprocessor Link',
  'contradicts': 'Red-Flag Conflict',
};
...
const TIER_LABELS: Record<string, string> = {
  'mod-001': 'Tier 1: Legal',
  'mod-002': 'Tier 2: Cyber',
  'mod-003': 'Tier 3: Escrow',
  'mod-004': 'Tier 4: MSA',
};
...
export default function KnowledgeGraphView() {
  ...
  return (
    <div className="relative h-full flex flex-col">
      {/* Legend */}
      <div className="flex items-center gap-4 px-4 py-2.5 border-b border-black/6 flex-wrap bg-white/40">
        <span className="text-[10px] font-mono font-bold text-[#64748b] uppercase tracking-wider">Relations:</span>
        {Object.entries(RELATION_COLORS).map(([rel, color]) => (
          <div key={rel} className="flex items-center gap-1.5">
            <div className="w-3.5 h-0.5 rounded-full" style={{ background: color }} />
            <span className="text-[10px] font-mono font-semibold text-[#0c2340]">
              {RELATION_LABELS[rel as GraphEdge['relation']] || rel}
            </span>
          </div>
        ))}
        ...
        <div className="flex items-center gap-3 pl-3 border-l border-black/10">
          <div className="flex items-center gap-1.5">
            <div className="w-2.5 h-2.5 rounded-[3px] bg-[#0284c7]" />
            <span className="text-[10px] font-mono font-semibold text-[#0c2340]">Vendor Entity</span>
          </div>
          <div className="flex items-center gap-1.5">
            <div className="w-2 h-2 rotate-45 bg-[#059669]" />
            <span className="text-[10px] font-mono font-semibold text-[#0c2340]">Compliance Gate</span>
          </div>
        </div>

        <div className="ml-auto flex items-center gap-3">
          {['mod-001', 'mod-002', 'mod-003', 'mod-004'].map((mod) => (
            <div key={mod} className="flex items-center gap-1.5">
              <div className="w-2.5 h-2.5 rounded-full" style={{ background: MODULE_COLORS[mod] }} />
              <span className="text-[10px] font-mono font-semibold text-[#64748b]">{TIER_LABELS[mod]}</span>
            </div>
          ))}
        </div>
      </div>
