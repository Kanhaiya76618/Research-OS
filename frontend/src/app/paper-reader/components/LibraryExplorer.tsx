...
export default function LibraryExplorer({ onSelectPaper }: { onSelectPaper?: (p: Paper) => void }) {
  ...
  const [breadcrumb] = useState(['CloudGate Infra', 'Tier-1 Diligence', 'Agreements']);
  ...
  return (
    <div
      className="clay-card overflow-hidden h-full flex flex-col"
      style={{
    ...
    >
      ...
      <div className="border-b border-black/6 pb-2">
        {/* Breadcrumb */}
        <div className="flex items-center gap-1 px-4 pt-3 pb-1 overflow-hidden">
          {breadcrumb.map((crumb, i) => (
            <React.Fragment key={crumb}>
              <span
                className={`text-[10px] font-mono whitespace-nowrap ${
                  i === breadcrumb.length - 1 ? 'text-[#0c2340] font-bold shrink-0' : 'text-[#64748b] truncate min-w-0'
                }`}
              >
                {crumb}
              </span>
              {i < breadcrumb.length - 1 && <ChevronRight size={10} className="text-[#64748b] shrink-0" />}
            </React.Fragment>
          ))}
        </div>
        ...
        <div className="flex items-center gap-2 px-4 py-1.5">
          {/* Filter */}
          <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl clay-input flex-1 min-w-0">
            <Search size={12} className="text-[#64748b] shrink-0" />
            <input
              type="text"
              value={filter}
              onChange={(e) => setFilter(e.target.value)}
              placeholder="Search agreements & certs…"
              aria-label="Filter documents"
              className="bg-transparent text-xs text-[#0c2340] placeholder-[#94a3b8] outline-none w-full min-w-0 font-medium"
            />
          </div>

          {/* View switcher */}
          <div className="flex items-center gap-0.5 p-1 rounded-xl bg-slate-100/80 border border-slate-200/60 shrink-0">
            {[
              ...
              return (
                <button
                  ...
                  className={`p-1.5 rounded-lg transition-all duration-150 ${active ? 'bg-white shadow-sm text-[#0284c7] font-bold' : 'text-[#64748b] hover:text-[#0c2340]'}`}
                  aria-label={v.label}
                >
